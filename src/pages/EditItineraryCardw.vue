<template>
    <div>
        <div class="gt-sm">
            <div class="column" style="min-width: 300px;max-width: 600px;background-color: white;">
                <div style="padding: 20px 20px 0px 20px;">
                <q-card class="row" style="width:100%;background-color: #F4F4F4;">
                    <div class="column" style="width: 70%;">
                        <div class="row" style="width: 100%;">
                            <div style="width: 15%;padding: 10px;">
                                <q-avatar>
                                    <img src="../assets/editcard/avatar2.svg" alt="">
                                </q-avatar>
                            </div>
                            <div class="column" style="width: 80%;padding: 10px;">
                                <div class="custom_itinerary_text1">Simran Chaudhary</div>
                                <div class="custom_itinerary_text2">Color Holidays</div>
                                <q-rating v-model="ratingModel" size="1em" :max="5" color="#FDD836" />
                            </div>
                        </div>
                        <div style="padding: 8px;">
                            <div class="custom_itinerary_text3" style="padding:10px;">
                                Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                Ipsum is simply dummy
                                text of the
                                printing and typesetting industry.
                            </div>
                        </div>
                    </div>
                    <div style="width: 30%;">
                        <div style="justify-content: center;">
                            <q-btn icon="question_answer" v-bind:disable="true" style="width: auto;background-color: #003FA3;color: white;">
                                <div class="custom_btn_text" style="padding-left: 3px;">Message Us</div>
                                <!-- <div style="height: auto;width:auto; max-width: 100%;justify-content: center;margin-left: 5px;">
                                    <img src="../assets/editcard/whatsapp.svg" alt="" style="width: 70%;height: 70%;">
                                </div> -->
                            </q-btn>
                        </div>
                    </div>
                </q-card>
                </div>
                <div style="padding: 14px 20px 0px 20px;;">
                <q-card class="row" style="width:100%;background-color: #F4F4F4;">
                    <div class="column" style="width: 70%;">
                        <div class="row" style="width: 100%;">
                            <div style="width: 15%;padding: 10px;">
                                <q-avatar>
                                    <img src="../assets/editcard/avatar2.svg" alt="">
                                </q-avatar>
                            </div>
                            <div class="column" style="width: 80%;padding: 10px;">
                                <div class="custom_itinerary_text1">Simran Chaudhary</div>
                                <div class="custom_itinerary_text2">Color Holidays</div>
                                <q-rating v-model="ratingModel" size="1em" :max="5" color="#FDD836" />
                            </div>
                        </div>
                        <div style="padding: 8px;">
                            <div class="custom_itinerary_text3" style="padding:10px;">
                                Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                Ipsum is simply dummy
                                text of the
                                printing and typesetting industry.
                            </div>
                        </div>
                    </div>
                    <div style="width: 30%;">
                        <div style="justify-content: center;">
                            <q-btn icon="question_answer" v-bind:disable="true" style="width: auto;background-color: #003FA3;color: white;">
                                <div class="custom_btn_text" style="padding-left: 3px;">Message Us</div>
                            </q-btn>
                        </div>
                    </div>
                </q-card>
                </div>
                <div style="padding: 14px 20px 20px 20px;">
                <q-card class="row" style="width:100%;background-color: #F4F4F4;">
                    <div class="column" style="width: 70%;">
                        <div class="row" style="width: 100%;">
                            <div style="width: 15%;padding: 10px;">
                                <q-avatar>
                                    <img src="../assets/editcard/avatar2.svg" alt="">
                                </q-avatar>
                            </div>
                            <div class="column" style="width: 80%;padding: 10px;">
                                <div class="custom_itinerary_text1">Simran Chaudhary</div>
                                <div class="custom_itinerary_text2">Color Holidays</div>
                                <q-rating v-model="ratingModel" size="1em" :max="5" color="#FDD836" />
                            </div>
                        </div>
                        <div style="padding: 8px;">
                            <div class="custom_itinerary_text3" style="padding:10px;">
                                Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                Ipsum is simply dummy
                                text of the
                                printing and typesetting industry.
                            </div>
                        </div>
                    </div>
                    <div style="width: 30%;">
                        <div style="justify-content: center;">
                            <q-btn icon="question_answer" v-bind:disable="true" style="width: auto;background-color: #003FA3;color: white;">
                                <div class="custom_btn_text" style="padding-left: 3px;">Message Us</div>
                            </q-btn>
                        </div>
                    </div>
                </q-card>
                </div>
            </div>
            <!-- <q-card flat bordered class="container70 absolute-center">
                <div class="card79">
                    <img src="../assets/editcard/cancelbtn.svg" alt="">
                </div>
                <div class="coontainer71">
                    <q-card flat bordered class="card70">
                        <div class="card71">
                            <div class="card72">
                                <div class="card73">
                                    <div class="card74">
                                        <div class="card75">
                                            <img src="../assets/editcard/avatar.svg" alt="">
                                        </div>
                                        <div class="card76">
                                            <text class="text70">Praveen Dubey</text>
                                            <text class="text71">Dubey Travelers</text>
                                            <div class="star">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="card77">
                                    <text class="text72">
                                        Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                        Ipsum is simply dummy text of the
                                        printing and typesetting industry.
                                    </text>
                                </div>
                            </div>
                            <div class="btn70">
                                <div class="card78">
                                    <img src="../assets/editcard/whatsapp.svg" alt="">
                                    <text class="text73">Message US</text>
                                </div>
                            </div>
                        </div>
                    </q-card>
                    <q-card flat bordered class="card70">
                        <div class="card71">
                            <div class="card72">
                                <div class="card73">
                                    <div class="card74">
                                        <div class="card75">
                                            <img src="../assets/editcard/avatar2.svg" alt="">
                                        </div>
                                        <div class="card76">
                                            <text class="text70">Simran Chaudhary</text>
                                            <text class="text71">Color Holidays</text>
                                            <div class="star">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star2.svg" alt="">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="card77">
                                    <text class="text72">
                                        Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                        Ipsum is simply dummy
                                        text of the
                                        printing and typesetting industry.
                                    </text>
                                </div>
                            </div>
                            <div class="btn70">
                                <div class="card78">
                                    <img src="../assets/editcard/whatsapp.svg" alt="">
                                    <text class="text73">Message US</text>
                                </div>
                            </div>
                        </div>
                    </q-card>
                    <q-card flat bordered class="card70">
                        <div class="card71">
                            <div class="card72">
                                <div class="card73">
                                    <div class="card74">
                                        <div class="card75">
                                            <img src="../assets/editcard/avatar3.svg" alt="">
                                        </div>
                                        <div class="card76">
                                            <text class="text70">Payal Kapoor</text>
                                            <text class="text71">Travel Connections</text>
                                            <div class="star">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star.svg" alt="">
                                                <img src="../assets/editcard/Star2.svg" alt="">
                                                <img src="../assets/editcard/Star2.svg" alt="">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="card77">
                                    <text class="text72">
                                        Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                        Ipsum is simply dummy
                                        text of the
                                        printing and typesetting industry.
                                    </text>
                                </div>
                            </div>
                            <div class="btn70">
                                <div class="card78">
                                    <img src="../assets/editcard/whatsapp.svg" alt="">
                                    <text class="text73">Message US</text>
                                </div>
                            </div>
                        </div>
                    </q-card>
                </div>
            </q-card> -->
        </div>
        <div class="lt-md" style="max-width: 300px;max-height: 500px;">
            <div style="margin: auto;background-color:white;padding: 10px;height: 100%;">
                <!-- <q-card flat bordered class="card92">
                    <div class="card93">
                        <div class="card94">
                            <div class="card96">
                                <div class="card98">
                                    <img src="../assets/editcard/Avatar4.svg" alt="">
                                    <div class="card99">
                                        <text class="text90">Praveen Dubey</text>
                                        <text class="text91">Dubey Travelers</text>
                                        <div class="card1">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="card97">
                                <text class="text92">
                                    Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                    Ipsum is
                                    simply dummy text of the.
                                </text>
                            </div>
                        </div>
                        <div class="card95">
                            <text class="text93">Message US</text>
                            <div class="container91" style="margin-bottom: 15px;">
                                <img src="../assets/editcard/whatsapp.svg" alt="">
                            </div>
                        </div>
                    </div>
                </q-card>
                <q-card flat bordered class="card92">
                    <div class="card93">
                        <div class="card94">
                            <div class="card96">
                                <div class="card98">
                                    <img src="../assets/editcard/Avatar4.svg" alt="">
                                    <div class="card99">
                                        <text class="text90">Simran Chaudhary</text>
                                        <text class="text91">Color Holidays</text>
                                        <div class="card1">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star2.svg" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="card97">
                                <text class="text92">
                                    Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                    Ipsum is
                                    simply dummy
                                    text of the.
                                </text>
                            </div>
                        </div>
                        <div class="card95">
                            <text class="text93">Message US</text>
                            <div class="container91" style="margin-bottom: 15px;">
                                <img src="../assets/editcard/whatsapp.svg" alt="">
                            </div>
                        </div>
                    </div>
                </q-card>
                <q-card flat bordered class="card92">
                    <div class="card93">
                        <div class="card94">
                            <div class="card96">
                                <div class="card98">
                                    <img src="../assets/editcard/Avatar4.svg" alt="">
                                    <div class="card99">
                                        <text class="text90">Payal Kapoor</text>
                                        <text class="text91">Travel Connections</text>
                                        <div class="card1">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star.svg" alt="">
                                            <img src="../assets/editcard/Star2.svg" alt="">
                                            <img src="../assets/editcard/Star2.svg" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="card97">
                                <text class="text92">
                                    Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                    Ipsum is
                                    simply dummy
                                    text of the.
                                </text>
                            </div>
                        </div>
                        <div class="card95">
                            <text class="text93">Message US</text>
                            <div class="container91" style="margin-bottom: 15px;">
                                <img src="../assets/editcard/whatsapp.svg" alt="">
                            </div>
                        </div>
                    </div>
                </q-card> -->
                <div style="padding: 10px 10px 0 10px;">
                    <q-card class="column" style="background-color: #F4F4F4;padding: 10px 10px 0 10px;">
                        <div class="row" style="width: 100%;">
                            <div style="width: 20%;">
                                <q-avatar>
                                    <img src="../assets/editcard/avatar2.svg" alt="">
                                </q-avatar>
                            </div>
                            <div class="column" style="width: 80%;">
                                <div class="custom_itinerary_text1">Simran Chaudhary</div>
                                <div class="custom_itinerary_text2">Color Holidays</div>
                                <q-rating v-model="ratingModel" size="1em" :max="5" color="#FDD836" />
                            </div>
                        </div>
                        <div style="padding: 8px;">
                            <div class="custom_itinerary_text3">
                                Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                Ipsum is simply dummy
                                text of the
                                printing and typesetting industry.
                            </div>
                        </div>
                        <div class="" style="width: 100%;padding: 0 0 10px 0;">
                            <q-btn icon="question_answer" v-bind:disable="true" class="row full-width" style="width: auto;background-color: #003FA3;justify-content: center;color: #F4F4F4;">
                                <div class="custom_btn_text" style="margin-left: 10px;">Message Us</div>
                                <!-- <div style="height: auto;width:auto; max-width: 100%;justify-content: center;margin-left: 5px;">
                                <img src="../assets/editcard/whatsapp.svg" alt="" style="width: 70%;height: 70%;">
                            </div> -->
                            </q-btn>
                        </div>
                    </q-card>
                </div>
                <div style="padding: 0 10px 0 10px;">
                    <q-card class="column" style="background-color: #F4F4F4;padding: 10px 10px 0 10px;margin-top: 20px;">
                        <div class="row" style="width: 100%;">
                            <div style="width: 20%;">
                                <q-avatar>
                                    <img src="../assets/editcard/avatar2.svg" alt="">
                                </q-avatar>
                            </div>
                            <div class="column" style="width: 80%;">
                                <div class="custom_itinerary_text1">Simran Chaudhary</div>
                                <div class="custom_itinerary_text2">Color Holidays</div>
                                <q-rating v-model="ratingModel" size="1em" :max="5" color="#FDD836" />
                            </div>
                        </div>
                        <div style="padding: 8px;">
                            <div class="custom_itinerary_text3">
                                Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                Ipsum is simply dummy
                                text of the
                                printing and typesetting industry.
                            </div>
                        </div>
                        <div class="" style="width: 100%;padding: 0 0 10px 0;">
                            <q-btn icon="question_answer" v-bind:disable="true" class="row full-width" style="width: auto;background-color: #003FA3;justify-content: center;color: #F4F4F4;">
                                <div class="custom_btn_text" style="margin-left: 10px;">Message Us</div>
                            </q-btn>
                        </div>
                    </q-card>
                </div>
                <div style="padding: 0 10px 10px 10px;">
                    <q-card class="column" style="background-color: #F4F4F4;padding: 10px 10px 0 10px;margin-top: 20px;">
                        <div class="row" style="width: 100%;">
                            <div style="width: 20%;">
                                <q-avatar>
                                    <img src="../assets/editcard/avatar2.svg" alt="">
                                </q-avatar>
                            </div>
                            <div class="column" style="width: 80%;">
                                <div class="custom_itinerary_text1">Simran Chaudhary</div>
                                <div class="custom_itinerary_text2">Color Holidays</div>
                                <q-rating v-model="ratingModel" size="1em" :max="5" color="#FDD836" />
                            </div>
                        </div>
                        <div style="padding: 8px;">
                            <div class="custom_itinerary_text3">
                                Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem
                                Ipsum is simply dummy
                                text of the
                                printing and typesetting industry.
                            </div>
                        </div>
                        <div class="" style="width: 100%;padding: 0 0 10px 0;">
                            <q-btn icon="question_answer" v-bind:disable="true" class="row full-width" style="width: auto;background-color: #003FA3;justify-content: center;color: #F4F4F4;">
                                <div class="custom_btn_text" style="margin-left: 10px;">Message Us</div>
                            </q-btn>
                        </div>
                    </q-card>
                </div>
            </div>
        </div>
    </div>
</template>
<script>
import { defineComponent } from 'vue';

export default defineComponent({
    name: 'EditItineraryCardw',
    mounted(){
        console.log(window.location.href)
        var param = this.url_params("place",window.location.href)
        
    },
    methods:{
        url_params( name, url ) {
            if (!url) url = location.href;
            name = name.replace(/[\[]/,"\\\[").replace(/[\]]/,"\\\]");
            var regexS = "[\\?&]"+name+"=([^&#]*)";
            var regex = new RegExp( regexS );
            var results = regex.exec( url );
            return results == null ? null : results[1];
        }
    }
})
</script>
<style>
.card79 {
    display: flex;
    flex-direction: row;
    align-items: flex-start;
    padding: 0px;
    gap: 10px;

    position: absolute;
    width: 40px;
    height: 40px;
    left: 820px;
    top: -20px;

    filter: drop-shadow(0px 0px 6px rgba(0, 0, 0, 0.25));
}

.card78 {
    display: flex;
    flex-direction: row;
    align-items: center;
    padding: 0px;
    gap: 10px;

    width: 140px;
    height: 32px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.text73 {
    width: 98px;
    height: 24px;

    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 16px;
    line-height: 24px;
    /* identical to box height */
    text-align: center;

    color: #FFFFFF;


    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.btn70 {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    padding: 10px 28px;
    gap: 10px;

    width: 196px;
    height: 52px;


    /* Travel Rover/Primary Color */
    background: #003FA3;
    border-radius: 30px;

    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.text72 {
    width: 568px;
    height: 72px;

    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 16px;
    line-height: 24px;

    color: #757575;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card77 {

    flex-direction: row;
    align-items: flex-start;
    padding: 0px 0px 0px 10px;
    gap: 10px;

    width: 568px;
    height: 72px;


    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.star {
    display: flex;
    flex-direction: row;
    align-items: flex-start;
    padding: 0px;
    gap: 5px;

    width: 100px;
    height: 16px;


    /* Inside auto layout */
    flex: none;
    order: 2;
    flex-grow: 0;
}

.text71 {
    width: 224px;
    height: 21px;

    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 14px;
    line-height: 21px;
    right: 100px;
    /* identical to box height */


    color: #000000;


    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.text70 {
    width: 253px;
    height: 30px;

    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 20px;
    line-height: 30px;
    /* identical to box height */


    color: #000000;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card76 {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    padding: 0px;
    gap: 1px;

    width: 453px;
    height: 69px;


    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.card75 {
    display: flex;
    flex-direction: row;
    align-items: flex-start;
    padding: 0px;
    gap: 3.46px;

    width: 40px;
    height: 40px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card74 {
    display: flex;
    flex-direction: row;
    align-items: flex-start;
    padding: 0px;
    gap: 10px;

    width: 203px;
    height: 69px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card73 {
    display: flex;
    flex-direction: row;
    justify-content: space-between;
    align-items: flex-start;
    padding: 0px;
    gap: 10px;

    width: 440px;
    height: 65px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card72 {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    padding: 0px;
    gap: 20px;

    width: 500px;
    height: 157px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card71 {
    display: flex;
    flex-direction: row;
    justify-content: space-between;
    align-items: center;
    padding: 0px;
    gap: 30px;

    width: 772px;
    height: 157px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    align-self: stretch;
    flex-grow: 0;
}

.card70 {
    box-sizing: border-box;

    /* Auto layout */
    display: flex;
    flex-direction: column;
    justify-content: flex-end;
    align-items: flex-start;
    padding: 10px;
    gap: 10px;

    width: 792px;
    height: 177px;

    background: #F4F4F4;
    border: 1px solid #E6E7EA;
    border-radius: 10px;

    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.coontainer71 {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    padding: 0px;
    gap: 10px;

    position: absolute;
    width: 792px;
    height: 551px;
    left: 25px;
    top: 25px;
}

.container70 {
    position: relative;
    width: 842px;
    height: 601px;

    background: #FFFFFF;
    border-radius: 16px;
}

/* edititinerarycardmobile */

.container91 {
    width: 20.13px;
    height: 20.13px;


    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.text93 {
    width: 97px;
    height: 24px;

    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 16px;
    line-height: 24px;
    /* identical to box height */
    text-align: center;

    color: #FFFFFF;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.text92 {
    width: 338px;
    height: 72px;

    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 16px;
    line-height: 24px;

    color: #757575;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 1;
}

.card1 {
    display: flex;
    flex-direction: row;
    align-items: flex-start;
    padding: 0px;
    gap: 5px;

    width: 100px;
    height: 16px;


    /* Inside auto layout */
    flex: none;
    order: 2;
    flex-grow: 0;
}

.text91 {
    width: 314px;
    height: 21px;

    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 14px;
    line-height: 21px;
    /* identical to box height */

    color: #000000;


    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.text90 {
    width: 353px;
    height: 30px;

    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 20px;
    line-height: 30px;
    /* identical to box height */

    color: #000000;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card99 {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    padding: 0px;
    gap: 1px;

    width: 453px;
    height: 69px;


    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.card98 {
    display: flex;
    flex-direction: row;
    align-items: flex-start;
    padding: 0px;
    gap: 10px;

    width: 503px;
    height: 69px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card97 {
    display: flex;
    flex-direction: row;
    align-items: flex-start;
    padding: 0px 0px 0px 10px;
    gap: 10px;

    width: 348px;
    height: 72px;


    /* Inside auto layout */
    flex: none;
    order: 1;
    flex-grow: 0;
}

.card96 {
    display: flex;
    flex-direction: row;
    align-items: flex-start;
    padding: 0px;
    gap: 10px;

    width: 348px;
    height: 65px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    align-self: stretch;
    flex-grow: 0;
}

.card95 {
    display: flex;
    flex-direction: row;
    justify-content: center;
    align-items: center;
    padding: 6.29032px 25.1613px;
    gap: 13.21px;

    width: 348px;
    height: 40px;

    /* Travel Rover/Primary Color */
    background: #003FA3;
    border-radius: 3.77419px;

    /* Inside auto layout */
    flex: none;
    order: 1;
    align-self: stretch;
    flex-grow: 0;
}

.card94 {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    padding: 0px;
    gap: 20px;

    width: 348px;
    height: 157px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card93 {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    padding: 0px;
    gap: 20px;

    width: 348px;
    height: 217px;


    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card92 {
    box-sizing: border-box;

    /* Auto layout */
    display: flex;
    flex-direction: column;
    justify-content: flex-end;
    align-items: flex-start;
    padding: 10px;
    gap: 10px;

    width: 368px;
    height: 237px;

    background: #F4F4F4;
    border: 1px solid #E6E7EA;
    border-radius: 10px;

    /* Inside auto layout */
    flex: none;
    order: 0;
    flex-grow: 0;
}

.card91 {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    padding: 0px;
    gap: 20px;

    position: absolute;
    width: 368px;
    height: 563px;
    left: 20px;
    top: 20px;
    overflow-y: scroll;
}

.card90 {
    position: relative;
    width: 408px;
    height: 603px;

    background: #FFFFFF;
    box-shadow: 0px 0px 34px rgba(0, 0, 0, 0.25);
}
.custom_btn_text {
    color: white;
}

.custom_itinerary_text1 {
    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 16px;
    line-height: 30px;

    color: #000000;
}

.custom_itinerary_text2 {
    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 12px;
    line-height: 21px;

    color: #000000;
}

.custom_itinerary_text3 {
    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 14px;
    line-height: 24px;

    color: #757575;
}

.custom_btn_text {
    font-family: 'Poppins';
    font-style: normal;
    font-weight: 500;
    font-size: 14px;
    line-height: 24px;
    text-align: center;

    color: #FFFFFF;
}
</style>